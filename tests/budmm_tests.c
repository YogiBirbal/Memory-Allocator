...
Test(bud_malloc_suite, malloc_invalid_size, .init = bud_mem_init, .fini = bud_mem_fini,
     .timeout = 5) {
    errno = 0;

    int *x = bud_malloc(MAX_BLOCK_SIZE + 1);
    cr_assert_null(x);
...
Test(bud_realloc_suite, realloc_Invalid_pointer, .init = bud_mem_init, .fini = bud_mem_fini,
     .timeout = 5,.signal = SIGABRT) {
    errno = 0;
    int *x  = bud_malloc(100);
    bud_free(x);

    void *y = bud_realloc(x, 40);
    (void) y;

    //bud_header *bhdr = PAYLOAD_TO_HEADER(x);
    //bhdr -> order = bhdr-> order + 1;

    //expect_errno_value(0);
}

Test(bud_realloc_suite, realloc_NULL_pointer, .init = bud_mem_init, .fini = bud_mem_fini,
     .timeout = 5) {
    errno = 0;

    int *x  = bud_malloc(80); //ORDER = 7
    //bud_free(x);
    int *y = bud_realloc(NULL, 80);
    bud_header *bhdrx = PAYLOAD_TO_HEADER(x);
    bud_header *bhdry = PAYLOAD_TO_HEADER(y);
    bud_free(x);
    bud_free(y);


    bud_free_block *a = (bud_free_block *)((uint64_t)bhdrx ^(128 ));
    cr_assert_eq(a, bhdry,"These two blocks should be buddies");


}

Test(bud_malloc_suite, malloc_check_buddy_list, .init = bud_mem_init, .fini = bud_mem_fini,
     .timeout = 5) {
    errno = 0;


    void *right= bud_malloc(MIN_BLOCK_SIZE - sizeof(bud_header));
    void *left = bud_malloc(MIN_BLOCK_SIZE - sizeof(bud_header) - 1);
    bud_header *rightheader = PAYLOAD_TO_HEADER(right);
    bud_header *leftheader = PAYLOAD_TO_HEADER(left);
...
*/
    ...
    assert_nonempty_free_list(ORDER_MIN + 1);
    assert_nonempty_free_list(ORDER_MIN + 2);


 bud_free_block *rightplusonebud = (bud_free_block *)((uint64_t)rightheader ^(ORDER_TO_BLOCK_SIZE(ORDER_MIN + 1)));

    cr_assert_eq(free_list_heads[1].next,
           rightplusonebud,
          "Right order + 1 buddy should be in This list");

    bud_free_block *rightbud = (bud_free_block *)((uint64_t)rightheader ^(ORDER_TO_BLOCK_SIZE(ORDER_MIN)));

    cr_assert_eq(leftheader,
           rightbud,
          "Right buddy should be in Left");




    void *big = bud_malloc(ORDER_TO_BLOCK_SIZE(ORDER_MIN + 2) - sizeof(bud_header));
    void *down = bud_malloc(ORDER_TO_BLOCK_SIZE(ORDER_MIN + 2) - sizeof(bud_header));
    void *up = bud_malloc(ORDER_TO_BLOCK_SIZE(ORDER_MIN + 2) - sizeof(bud_header));
    bud_header *bigheader = PAYLOAD_TO_HEADER(big);
    bud_header *downheader = PAYLOAD_TO_HEADER(down);
    bud_header *upheader = PAYLOAD_TO_HEADER(up);

/*
        Right | Left ||         |||    Big      |||    Down      |     Up
          5       5        6            7               7               7
*/

    assert_empty_free_list(ORDER_MIN);
    assert_nonempty_free_list(ORDER_MIN + 1);
    assert_empty_free_list(ORDER_MIN + 2);
    assert_empty_free_list(ORDER_MIN + 3);
    assert_nonempty_free_list(ORDER_MIN + 4);
    bud_free_block *rightplus2bud = (bud_free_block *)((uint64_t)rightheader ^(ORDER_TO_BLOCK_SIZE(ORDER_MIN + 2)));

    cr_assert_eq(bigheader,
           rightplus2bud,
          "Right order + 2 buddy should be Big");

    bud_free_block *downbud = (bud_free_block *)((uint64_t)downheader ^(ORDER_TO_BLOCK_SIZE(ORDER_MIN + 2)));

    cr_assert_eq(upheader,
           downbud,
          "Up buddy should be Down");




    void *pikachu = bud_malloc(MIN_BLOCK_SIZE - sizeof(bud_header) + 1);
    void *pichu = bud_malloc(ORDER_TO_BLOCK_SIZE(ORDER_MIN + 4) - sizeof(bud_header));
    bud_header *pikachuheader = PAYLOAD_TO_HEADER(pikachu);
    bud_header *pichuheader = PAYLOAD_TO_HEADER(pichu);


/*
        Right | Left ||   Pikachu   |||    Big      |||    Down      |     Up        ||||    Pichu
          5       5        6                 7               7               7               9
*/

    assert_empty_free_list(ORDER_MIN);
    assert_empty_free_list(ORDER_MIN + 1);
    assert_empty_free_list(ORDER_MIN + 2);
    assert_empty_free_list(ORDER_MIN + 3);
    assert_empty_free_list(ORDER_MIN + 4);



    bud_free_block *pichubud = (bud_free_block *)((uint64_t)pichuheader ^(ORDER_TO_BLOCK_SIZE(ORDER_MIN + 4)));

    cr_assert_eq(rightheader,
           pichubud,
          "Pichu bud should be right");
...
Test(bud_free_suite, free_check_buddy_lists, .init = bud_mem_init, .fini = bud_mem_fini,
     .timeout = 5) {
    errno = 0;

    void *peanutbutter = bud_malloc(MIN_BLOCK_SIZE - sizeof(bud_header));
    void *jelly = bud_malloc(MIN_BLOCK_SIZE - sizeof(bud_header) - 1);

    void *cookies = bud_malloc(MIN_BLOCK_SIZE - sizeof(bud_header));
    void *milk = bud_malloc(MIN_BLOCK_SIZE - sizeof(bud_header) - 1);

    void *plusle = bud_malloc(MIN_BLOCK_SIZE - sizeof(bud_header) + 1);
    void *minun = bud_malloc(MIN_BLOCK_SIZE - sizeof(bud_header) + 1);

/*

    PeanutButter | Jelly || Cookies | Milk ||| Plusle | Minun |
          5          5         5        5         6       6
*/
    ...
    bud_header *milkheader = PAYLOAD_TO_HEADER(milk);
    bud_header *jellyheader = PAYLOAD_TO_HEADER(jelly);
    bud_header *cookiesheader = PAYLOAD_TO_HEADER(cookies);
    bud_header *peanutheader = PAYLOAD_TO_HEADER(peanutbutter);
    bud_header *plusheader = PAYLOAD_TO_HEADER(plusle);
    bud_header *minunheader = PAYLOAD_TO_HEADER(minun);


    bud_free_block *milkbud = (bud_free_block *)((uint64_t)milkheader ^(MIN_BLOCK_SIZE ));


    cr_assert_eq(free_list_heads[0].next->next,
           milkbud,
          "Free block here should be a buddy to milk");

    bud_free(milk);


    cr_assert_eq(free_list_heads[0].next,
           jellyheader,
          "Jelly should be in this list ");

    cr_assert_eq(free_list_heads[1].next,
           cookiesheader,
          "Cookies&Milk should be coalesced and put together here ");

    bud_free(peanutbutter);
